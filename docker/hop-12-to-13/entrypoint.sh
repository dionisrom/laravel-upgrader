#!/usr/bin/env bash
# ============================================================
# Entrypoint: hop-12-to-13
# Laravel 12 → Laravel 13 transformation pipeline
#
...
# ─── PHP 8.3 minimum guard ────────────────────────────────────────────────────
# Laravel 13 requires PHP 8.3+. The container image itself runs PHP 8.3,
# but we also check the workspace's composer.json constraint.

PHP_VERSION=$("${PHP_BIN}" -r 'echo PHP_MAJOR_VERSION . "." . PHP_MINOR_VERSION;')
if [ "$(printf '%s\n' "8.3" "${PHP_VERSION}" | sort -V | head -n1)" != "8.3" ]; then
    emit "{\"event\":\"config_error\",\"hop\":\"${HOP}\",\"ts\":$(ts),\"error\":\"PHP 8.3+ required for Laravel 13. Found PHP ${PHP_VERSION}\"}"
    exit 2
fi

# ─── PHP version guard: check workspace composer.json ─────────────────────────

if [ -f "${WORKSPACE}/composer.json" ]; then
    emit "{\"event\":\"stage_start\",\"stage\":\"PhpVersionGuard\",\"hop\":\"${HOP}\",\"ts\":$(ts)}"
    "${PHP_BIN}" "${SRC}/Rector/Rules/L12ToL13/PhpVersionGuard.php" "${WORKSPACE}/composer.json" || true
    emit "{\"event\":\"stage_complete\",\"stage\":\"PhpVersionGuard\",\"hop\":\"${HOP}\",\"ts\":$(ts),\"duration_ms\":0}"
fi
