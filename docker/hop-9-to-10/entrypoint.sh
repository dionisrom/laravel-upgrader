#!/usr/bin/env bash
# ============================================================
# Entrypoint: hop-9-to-10
# Laravel 9 → Laravel 10 transformation pipeline
#
...
# ─── PHP 8.1 minimum guard ────────────────────────────────────────────────────

PHP_VERSION=$("${PHP_BIN}" -r 'echo PHP_MAJOR_VERSION . "." . PHP_MINOR_VERSION;')
if [ "$(printf '%s\n' "8.1" "${PHP_VERSION}" | sort -V | head -n1)" != "8.1" ]; then
    emit "{\"event\":\"config_error\",\"hop\":\"${HOP}\",\"ts\":$(ts),\"error\":\"PHP 8.1+ required for Laravel 10. Found PHP ${PHP_VERSION}\"}"
    exit 2
fi
...
# Package rules are now activated inside rector.l9-to-l10.php at config load
